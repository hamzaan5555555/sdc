...
const NeonCursor = () => {
    ...
    const handleMouseOver = useCallback((e) => {
        ...
        if (target.matches('a, button, input, [data-hover="true"]')) {
            setIsHovering(true);
            void trailControls.start({ scale: 1.5, borderColor: 'rgb(255, 150, 50)', borderWidth: '3px' });
            void glowControls.start({ scale: 2, opacity: 0.8 });
        }
    }, [trailControls, glowControls]);

    const handleMouseOut = useCallback(() => {
        setIsHovering(false);
        void trailControls.start({ scale: 1, borderColor: 'rgb(236, 101, 23)', borderWidth: '2px' });
        void glowControls.start({ scale: 1, opacity: 0.4 });
    }, [trailControls, glowControls]);
    ...
    return (
        <div className="neon-cursor-container">
            <motion.div
                ...
                animate={{ x: position.x - 10, y: position.y - 10, scale: isClicking ? 0.8 : isHovering ? 1.2 : 1 }}
